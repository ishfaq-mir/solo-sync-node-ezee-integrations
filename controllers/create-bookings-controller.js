...
async function createABooking(body) {
  try {
    ...
    const processFlag = await markBookingProcessed(ReservationNo);
    return processFlag;
  } catch (error) {
    throw new Error(error);
  }
}
function enforceValidation(body) {
...
async function markBookingProcessed(reservationId) {
  const hotelId = 46138;
  let config = {
    method: "post",
